...
const getPromptForDecade = (decade: string) => `Create a high-detail, photorealistic image of the person in this photo, reimagined in the style of the ${decade}. The final image should have the quality, grain, and color grading of a professionally shot photograph from that era. Faithfully reproduce the clothing, hairstyle, and overall aesthetic of the ${decade}, ensuring the person is clearly visible and the final result is a high-quality, authentic-looking portrait.`;
...
function App() {
    ...
    const [viewMode, setViewMode] = useState<'app' | 'share'>('app');
    const [sharedContent, setSharedContent] = useState<{ sourceImage: string; decade: string } | null>(null);
    const [generatedSharedImage, setGeneratedSharedImage] = useState<GeneratedImage>({ status: 'pending' });
    const [isShareModalOpen, setIsShareModalOpen] = useState(false);
    const [shareModalData, setShareModalData] = useState<{ decade: string; imageUrl: string; shareUrl: string } | null>(null);

    // Effect to check for a share URL on initial load
    useEffect(() => {
        const params = new URLSearchParams(window.location.search);
        const shareParam = params.get('share');
        if (shareParam) {
            try {
                // Use replace to handle URL-safe base64 characters
                const decodedJson = atob(shareParam.replace(/_/g, '/').replace(/-/g, '+'));
                const data = JSON.parse(decodedJson);
                if (data.i && data.d) {
                    setSharedContent({ sourceImage: data.i, decade: data.d });
                    setViewMode('share');
                }
            } catch (e) {
                console.error("Failed to parse share data:", e);
                // Clean the URL if the share data is invalid
                window.history.replaceState({}, '', window.location.pathname);
            }
        }
    }, []);

    // Effect to generate the image when in share view mode
    useEffect(() => {
        if (viewMode === 'share' && sharedContent) {
            const generate = async () => {
                setGeneratedSharedImage({ status: 'pending' });
                try {
                    const prompt = getPromptForDecade(sharedContent.decade);
                    const resultUrl = await generateDecadeImage(sharedContent.sourceImage, prompt);
                    setGeneratedSharedImage({ status: 'done', url: resultUrl });
                } catch (err) {
                    const errorMessage = err instanceof Error ? err.message : "An unknown error occurred.";
                    setGeneratedSharedImage({ status: 'error', error: errorMessage });
                }
            };
            generate();
        }
    }, [viewMode, sharedContent]);
    ...
    const handleShareDecade = async (decade: string) => {
        const imageToShare = generatedImages[decade];
        
        // FIX: Add a more robust type guard to ensure imageToShare and its properties are defined.
        // This resolves TypeScript errors where `imageToShare` could be inferred incorrectly.
        if (!uploadedImage || !imageToShare || imageToShare.status !== 'done' || !imageToShare.url) {
            return;
        }
    
        try {
            // Resize source image to keep URL small, then create share payload
            const resizedSource = await resizeImage(uploadedImage, 400);
            const sharePayload = { i: resizedSource, d: decade };
            // URL-safe base64 encoding
            const encoded = btoa(JSON.stringify(sharePayload)).replace(/\+/g, '-').replace(/\//g, '_');
            const shareUrl = `${window.location.origin}${window.location.pathname}?share=${encoded}`;
    
            // Open modal with all necessary data
            setShareModalData({
                decade,
                imageUrl: imageToShare.url,
                shareUrl,
            });
    ...
    // Render a dedicated view for shared images
    if (viewMode === 'share' && sharedContent) {
        return (
            <main className="bg-black text-neutral-200 min-h-screen w-full flex flex-col items-center justify-center p-4 overflow-hidden relative">
                ...
                <div className="z-10 flex flex-col items-center justify-center w-full h-full flex-1">
                    <div className="text-center mb-10">
                        ...
                        <p className="font-permanent-marker text-neutral-300 mt-2 text-xl tracking-wide">A photo from the {sharedContent.decade}</p>
                    </div>
                    <PolaroidCard
                        caption={sharedContent.decade}
                        status={generatedSharedImage.status}
                        imageUrl={generatedSharedImage.url}
                        error={generatedSharedImage.error}
                        isMobile={true} // Force mobile styles for single card view
                    />
                    <a href={window.location.pathname} className={`${primaryButtonClasses} mt-8`}>
                        Create Your Own
                    </a>
                </div>
                <Footer />
